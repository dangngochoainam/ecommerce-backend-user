...
export class WorkflowCoreSQLService extends AbstractSQLService {
		...
		await m.getRepository(WorkflowEntity).update(
			{
				...
				status: Not(In(FINAL_WORKFLOW_STATUS.slice())),
			},
			{
				...(params.status
					? {
							status: params.status,
							finished: FINAL_WORKFLOW_STATUS.includes(
								params.status as (typeof FINAL_WORKFLOW_STATUS)[0],
							),
						}
					: {}),
	...
	public async sqlMarkAsExecuted(
		m: EntityManager | undefined = this.defaultManager,
		params: {
			workflowId: string;
		},
